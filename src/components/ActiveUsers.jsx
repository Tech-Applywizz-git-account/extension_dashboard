    ...
    const fetchUsers = async () => {
        ...
        try {
            ...
            const processedUsers = data
                .filter(user => {
                    // For "All Time" or default ranges — show everyone
                    // Use created_at for the join date filter (not updated_at)
                    if (!dateRange || dateRange === 'All Time' || dateRange === 'Last 7 Days' || dateRange === 'Last 30 Days') return true;
                    const joinDate = user.created_at || user.updated_at;
